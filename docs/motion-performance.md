...
- `scene-renderer.js` 绘制星点和尾迹，使用可随 GPU 压力调整的背景画布。
- `glyph-renderer.js` 在同一个 Worker 内绘制独立的原生分辨率文字层，只占文字包围区域。完整字形只上传一次，覆盖率变化时更新小型 alpha 蒙版和对应文字区域；静止后的文字不继续重绘。文字层随 Hero 原生滚动和淡出。
...
不按浏览器型号或显卡名称做特殊处理，也不使用固定的 180Hz 分支。Worker 遵循浏览器显示时钟，并用 WebGL2 fence 的零超时查询确认上一帧 GPU 工作是否完成，避免堆积未完成的绘制。连续 GPU 压力只降低背景画布的背板分辨率（不低于一个 CSS 像素对应一个画布像素），保留所有星点和细丝；稳定后恢复。文字层始终使用屏幕 DPR，不跟随背景降采样。fence 完成仍不等同于屏幕呈现，所以不把渲染标签当作流畅度证明。

文字层保留绘制缓冲区，用 scissor 只清除和重画改变的区域，同时重绘与该区域相交的字形，避免擦掉相邻笔画。坐标对齐设备像素，纹理坐标使用高精度计算。任一画布失效都恢复可读的 HTML 文字。

## 高清屏文字

原来字形纹理按原生 DPR 生成，但最终输出仍经过背景最高 1.5 DPR 的画布，导致高像素密度手机上的文字被放大、变软。现在文字的采样和输出都保留原生像素，背景分辨率独立调整。

`scripts/check-glyph-clarity.mjs` 在 1.25、2、3 DPR 的手机模拟中，将实际 GPU 输出与原始字形栅格逐像素比较；覆盖中文和英文。在 3 DPR 的两种语言样本中，笔画 alpha 与原始栅格一致。检查也确认文字静止后不重绘，以及模拟背景 GPU 压力时文字分辨率不变。这是渲染像素的验证，不代替真机性能测量。

## 同配置生产包对比

2026-09-11，Edge 152.0.4191.66，2550 × 1275，DPR 1.25，中文、新浏览器页面。以下是提交 `9084a78` 与 `94ab7c6` 的历史对比，尚未包含后续的原生分辨率文字层。两个版本从独立目录构建，使用同一套外部探针。依次采样开场约 6.2 秒和 Head 离场约 1.6 秒，不并行运行浏览器检查。
...
node scripts/check-glyph-clarity.mjs
